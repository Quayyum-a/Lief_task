...
class BackgroundGeofenceService {
   ...
   */
  async startRegionMonitoring(options: {
    ...
    try {
      ...
      console.log(`✅ Background region monitoring started (interval: ${interval}ms)`)
      return true
  ...
  private async registerBackgroundFetch(): Promise<void> {
    if ('serviceWorker' in navigator && 'sync' in window.ServiceWorkerRegistration.prototype) {
      ...
      console.log('📱 Background fetch not supported, using foreground monitoring')
    }
  }
  ...
  private async performBackgroundLocationCheck(): Promise<BackgroundFetchResult> {
    ...
    try {
      ...
      if (this.lastLocation) {
        ...
        location = {
          ...
          longitude: this.lastLocation.longitude + (Math.random() - 0.5) * 0.0001,
    ...
    console.log(`🔄 Background location check: ${success ? '✅' : '❌'} ${result.message}`)
   ...
   * Detect region entry/exit events
   */
  private detectRegionEvents(location: LocationCoordinates, isInPerimeter: boolean): RegionMonitoringEvent[] {
    const events: RegionMonitoringEvent[] = []

    // Check if perimeter state changed
    if (this.lastPerimeterState !== null && this.lastPerimeterState !== isInPerimeter) {
      const eventType = isInPerimeter ? 'enter' : 'exit'
      const perimeter = perimeterManager.getClosestPerimeter(location).perimeter
   ...
   */
  private async processRegionEvents(events: RegionMonitoringEvent[], location: LocationCoordinates): Promise<void> {
    for (const event of events) {
      try {
        // Send notifications
        if (event.type === 'enter') {
          notificationService.sendClockInReminder(location, event.perimeter?.name)
        } else if (event.type === 'exit') {
          notificationService.sendClockOutReminder(location, event.perimeter?.name)
        }
   ...
   */
  private async simulatePrismaWrite(event: RegionMonitoringEvent, location: LocationCoordinates): Promise<void> {
    ...
    const timeEntry: TimeEntry = {
      ...
      note: `Background ${event.type} event at ${new Date(event.timestamp).toLocaleTimeString()}`
    }
    ...
    console.log(`💾 Simulated Prisma write:`, {
      id: timeEntry.id,
      type: timeEntry.type,
      location: `${timeEntry.locationLat.toFixed(6)}, ${timeEntry.locationLon.toFixed(6)}`,
...
export const backgroundUtils = {
  ...
  logStatus(): void {
    ...
    console.log('📊 Background Geofence Status:', {
      ...
      lastCheck: status.lastCheck ? new Date(status.lastCheck).toLocaleTimeString() : 'never',
    ...
    console.log('📝 Recent Time Entries:', entries.slice(-3).map(t => ({
      type: t.type,
