...
class LocationService {
  ...
  private onLocationChange: ((location: LocationCoordinates) => void) | null = null
  ...
  async getCurrentPosition(options?: PositionOptions): Promise<LocationCoordinates> {
    return new Promise((resolve, reject) => {
      if (!this.isSupported()) {
        reject(this.createError('NOT_SUPPORTED', 'Geolocation is not supported by this browser'))
        return
      }
  ...
  startWatching(
    ...
    if (!this.isSupported()) {
      onError?.(this.createError('NOT_SUPPORTED', 'Geolocation is not supported'))
      return false
    }
  ...
  async requestPermission(): Promise<'granted' | 'denied' | 'prompt'> {
    ...
    if ('permissions' in navigator) {
      try {
        ...
        return this.testPermissionWithGetCurrentPosition()
      }
    }
   ...
   */
  private async testPermissionWithGetCurrentPosition(): Promise<'granted' | 'denied' | 'prompt'> {
    try {
  ...
  private mapGeolocationError(error: GeolocationPositionError): LocationError {
    switch (error.code) {
      case error.PERMISSION_DENIED:
        return this.createError('PERMISSION_DENIED', 'Location access was denied by the user')
      case error.POSITION_UNAVAILABLE:
        return this.createError('POSITION_UNAVAILABLE', 'Location information is unavailable')
      case error.TIMEOUT:
        return this.createError('TIMEOUT', 'The request to get user location timed out')
      default:
        return this.createError('POSITION_UNAVAILABLE', error.message || 'Unknown location error')
    }
  }
   ...
   */
  private createError(type: LocationError['type'], message: string): LocationError {
    const code = {
      PERMISSION_DENIED: 1,
