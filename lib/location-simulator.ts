...
export class LocationSimulator {
  ...
  startSequence(
    ...
    const processNext = () => {
      ...
      console.log(
        `📍 Simulated location ${this.currentIndex + 1}/${
  ...
  simulateWalk(
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      locations.push({ latitude: lat, longitude: lng, timestamp: Date.now() });
    ...
    const sequence: LocationSequence = {
      locations,
      durations,
      description: `Walking from (${from.latitude.toFixed(
        4
      )}, ${from.longitude.toFixed(4)}) to (${to.latitude.toFixed(
        4
...
export const testScenarios = {
  ...
  timesSquareTest: (): LocationSequence => ({
    locations: [
      ...
      { latitude: 40.7825, longitude: -73.97 }, // Southeast, inside
      { latitude: 40.782, longitude: -73.972 }, // Outside perimeter
...
export const simulatorUtils = {
  ...
  logPerimeterInfo(): void {
    ...
    perimeters.forEach((p, index) => {
      console.log(
        `   ${index + 1}. Center: (${p.centerLatitude.toFixed(
          6
        )}, ${p.centerLongitude.toFixed(6)}), Radius: ${p.radiusInMeters}m`
      );
    });
  },
};
